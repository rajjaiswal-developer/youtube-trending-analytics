...
# HELPER FUNCTIONS
# =======================
# Load and clean one country file
...
# Combined sentiment from title + tags
...
# =======================
# STEP 3: SEPARATE TITLE & TAGS SENTIMENT
# =======================
df['title_sentiment'] = df['title'].apply(lambda x: analyzer.polarity_scores(str(x))['compound'])
df['tags_sentiment'] = df['tags'].apply(lambda x: analyzer.polarity_scores(str(x))['compound'])
